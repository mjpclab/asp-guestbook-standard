...
if IsAccess then
	sql_adminstats_client_os=		"SELECT TOP 10 * FROM (SELECT os,Count(os) AS count_os FROM " &table_stats_clientinfo& " GROUP BY os ORDER BY Count(os) DESC) ORDER BY count_os DESC"
	sql_adminstats_client_browser=	"SELECT TOP 10 * FROM (SELECT browser,Count(browser) AS count_browser FROM " &table_stats_clientinfo& " GROUP BY browser ORDER BY Count(browser) DESC) ORDER BY count_browser DESC"
	sql_adminstats_client_screen=	"SELECT TOP 10 * FROM (SELECT screenwh,Count(screenwh) AS count_screenwh FROM (SELECT screenwidth &'*'& screenheight AS screenwh FROM " &table_stats_clientinfo& ") GROUP BY screenwh ORDER BY Count(screenwh) DESC) ORDER BY count_screenwh DESC"
	...
	sql_adminstats_client_source=	"SELECT TOP 10 * FROM (SELECT sourceaddr,Count(sourceaddr) AS count_sourceaddr FROM " &table_stats_clientinfo& " GROUP BY sourceaddr ORDER BY Count(sourceaddr) DESC) ORDER BY count_sourceaddr DESC"
elseif IsSqlServer then
	sql_adminstats_client_os=		"SELECT TOP 10 * FROM (SELECT TOP 100 PERCENT os,Count(os) AS count_os FROM " &table_stats_clientinfo& " GROUP BY os ORDER BY count_os DESC) AS __temp1 ORDER BY count_os DESC"
	sql_adminstats_client_browser=	"SELECT TOP 10 * FROM (SELECT TOP 100 PERCENT browser,Count(browser) AS count_browser FROM " &table_stats_clientinfo& " GROUP BY browser ORDER BY count_browser DESC) AS __temp1 ORDER BY count_browser DESC"
	...
	sql_adminstats_client_source=	"SELECT TOP 10 * FROM (SELECT TOP 100 PERCENT sourceaddr,Count(sourceaddr) AS count_sourceaddr FROM " &table_stats_clientinfo& " GROUP BY sourceaddr ORDER BY count_sourceaddr DESC) AS __temp1 ORDER BY count_sourceaddr DESC"
end if
