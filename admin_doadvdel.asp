...
case "1"
	if isdate(tparam) then
		...
		Call MessagePage("删除操作完成。","admin_advdel.asp")
	else
		Call MessagePage("您输入的日期有误，请检查。","admin_advdel.asp")
	end if
case "2"
	if isdate(tparam) then
		tparam=DateTimeStr(tparam)
...
case "3"
	if isnumeric(tparam) then
		if clng(tparam)>0 then
			...
			Call MessagePage("请输正确的数值。","admin_advdel.asp")
		end if
	else
		Call MessagePage("请输正确的数值。","admin_advdel.asp")
	end if
...
case "5"
	...
	if tparam<>"" then
		...
		Call MessagePage("不能输入空字符串或全部为通配符。","admin_advdel.asp")
	end if
case "6"
	tparam=replace(tparam,"'","''")
	tparam=replace(tparam,"[","[\[]")
