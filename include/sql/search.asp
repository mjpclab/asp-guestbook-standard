...
elseif IsSqlServer then
	sql_search_condition_reply=		"guestflag & 16=0 AND (guestflag & 32=0 OR guestflag & 64=0) AND id IN (SELECT articleid FROM " &table_reply& " WHERE reinfo LIKE N'%{0}%')"
	sql_search_condition_name=		"guestflag & 16=0 AND name LIKE N'%{0}%'"
	sql_search_condition_title=		"guestflag & 48=0 AND title LIKE N'%{0}%'"
	sql_search_condition_article=	"guestflag & 56=0 AND article LIKE N'%{0}%'"
	sql_search_count_inner=			"SELECT DISTINCT root_id FROM " &table_main& " WHERE "
